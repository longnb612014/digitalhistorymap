...
function searchProvince() {
  ...
  if (foundProvince || foundDataKey) {
    ...
    if (regionData) {
      ...
      if (currentMode === 'explore') {
        renderExploreFallback(foundProvince || dataKey, info);
      } else {
  ...
  .then(data => {
    L.geoJSON(data, {
      ...
      onEachFeature: function(feature, layer) {
        ...
        if (info.population) popupContent += `Dân số: ${info.population}<br>`;
        if (info.area) popupContent += `Diện tích: ${info.area}<br>`;
        if (info.description) popupContent += `Mô tả: ${info.description}`;
        ...
        layer.on('click', () => {
          ...
          if (currentMode === 'explore') {
            ...
            if (regionData) {
              ...
              renderExploreFallback(name, info);
            }
...
function renderExplorePanel(data) {
...
function renderExploreFallback(name, info) {
  const container = document.getElementById('explore-result');
  if (!container) return;
  container.innerHTML = `
    <div class="ep-header">
